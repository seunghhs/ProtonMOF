...
class MOFProtonModel(pl.LightningModule):
    def __init__(self, config: EasyDict, 
        ...
        self.moftransformer = get_moftransformer()


        self.chembert = ClassificationModel('roberta', 'seyonec/PubChem10M_SMILES_BPE_396_250', 
                                    num_labels=1,
                                    args={'evaluate_each_epoch': True, 
                                          'evaluate_during_training_verbose': True,
                                          'no_save': False, 'num_train_epochs': 10, 
                                          'regression' : True,
                                          'auto_weights': True}).model.roberta
    ...
    def forward(self,
        ...
        mof_embeds = self.moftransformer(batch)['cls_feats']
        guest_embeds=self.chembert(input_ids, attention_mask)[0][:,0,:]
        guest_embeds2=self.chembert(batch['input_ids2'], batch['attention_mask2'])[0][:,0,:]
        all_guest_embeds = (guest_embeds + guest_embeds2)/2

        
        mof_embeds = self.mof_dense(mof_embeds)
        all_guest_embeds = self.guest_dense(all_guest_embeds)
        t_embeds = self.t_dense(batch['T'])
        rh_embeds = self.rh_dense(batch['RH'])
        
        if (self.arr == 'arrhenius') :
            all_embeds = mof_embeds +  all_guest_embeds + rh_embeds
        else:
            all_embeds = mof_embeds +  all_guest_embeds + t_embeds + rh_embeds
...
def get_moftransformer():
    from moftransformer.config import config as _config
    from moftransformer import __root_dir__
    
    config = copy.deepcopy(_config())
    config['test_only'] = True
    config['load_path'] = str(__root_dir__) + '/database/moftransformer.ckpt'
    ...
    moftransformer = Module(_config)
    return moftransformer