...
from protonmof.model.model_finetune import MOFProtonModel
import torch
...
def main(args, fold):
    ...
    early_callback = EarlyStopping(monitor="avg_mae_loss", mode="min",patience=5,)
    ...
    train_loader =DataLoader(train_data, config.train.batch_size , num_workers = config.train.num_workers, collate_fn= collate_fn,
                             shuffle=True)    




    valid_loader =DataLoader(valid_data, config.train.batch_size , num_workers = config.train.num_workers, collate_fn= collate_fn,
                             shuffle=False)        
    ...
    if args.test:
        ...
        test_loader =DataLoader(test_data, config.train.batch_size , num_workers = config.train.num_workers, collate_fn= collate_fn,
                                 shuffle=False)   
...
if __name__ == '__main__':
    ...
    # data = pd.read_excel(config.dataset.proton_data_dir)
